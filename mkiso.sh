...
# than those used by default (/etc/yum.conf) to install/update
...
function cleanup_mounts {
    ...
    for _key in "${_keys[@]}" ; do
        if [ "${MKISO_VALS[$_key]}" ] ; then
            printf "Attempt unmount of %s\n" "${MKISO_VALS[$_key]}"
            if [ "$_key" == 'samba_mount_path' ] ; then
                # CIFS must be treated differently...
                sudo umount -i -f -t cifs "${MKISO_VALS[$_key]}" -l
                _rv=$?
            else
                sudo umount "${MKISO_VALS[$_key]}"
                _rv=$?
            fi
            if [ $_rv -eq 0 ] ; then
...
#
# check installed rpms to see if rquired tools are installed...
#
function check_installed_rpms {
    local _func=check_installed_rpms
    local _rpm
    local _val

    for _rpm in $REQUIRED_RPMS; do
        printf "%s: Check RPM %s...\n" $_func $_rpm
        _val=`rpm -qa | grep $_rpm`
        if [ $? -ne 0 ] ; then
            echo "-----------------------------------------------"
...
function mount_samba_fs {
    ...
    sudo mount -t cifs $_resource $_mount_path -o username=$_user
    if [ $? -ne 0 ] ; then
...
# Some arguments do not all {}s in the value
#
...
function proc_file_args {
    ...
    eval "declare -A _fileVals="${_fvstr}
...
function process_command {
    local _func='process_command'
    local argDefs
    declare -A array argDefs

    printf "%s: %s\n" $_func "$*"

    argDefs['create']='do_cmd_create'
    argDefs['test']='do_cmd_iso_test'
    argDefs['rpm2iso']='do_cmd_rpm_to_iso'
    argDefs['show']='do_cmd_show_params'
...
function check_path_safe {
    ...
    # lastly you must be based in the work-path
    if [[ ! "$_path" =~ ^$WORKSPACE ]] ; then
        printf "%s: %s must be child of %s\n" $_func $_path $WORKSPACE
        return 1
    fi
    # you must own the directory, ot a parent if it doesnt exist...
...
# purge_yum_state:
#
# $1 - IN: Name of associative parameter array
#
# Removes directories and files which form the yum cache state and
# downloaded RPMs.  
#
# NOTE: It would be preferable to not do this as root, but the yum
...
#
function purge_yum_state {
    local _myfunc=purge_yum_state
    local _pargs
    local _an=$1
    local _func=$2
    ...
    eval "readonly -A _pargs"=${_astr#*=}

    local _instRootPath=${_pargs[yum_install_path]}
    local _rpmDloadPath=${_pargs[yum_rpm_path]}
    ...
    if [ -d "$_instRootPath" ] ; then
        printf "%s: Purge YUM install state (%s)\n" $_func "$_instRootPath"
        check_path_safe "$_instRootPath" "$_func:$_myfunc"
        if [ $? -ne 0 ] ; then
            return 1
        fi
        printf "%s: sudo mv -f %s -> %s\n" $_myfunc "$_instRootPath" "$_instRootPath".old
        sudo mv -f "$_instRootPath" "$_instRootPath".old
        if [ $? -ne 0 ] ; then
            printf "%s: mv FAILED (%s)\n" $_myfunc "$_instRootPath"
            return 1
        fi
        printf "%s: sudo rm -rf %s\n" $_myfunc "$_instRootPath".old
        sudo rm -rf "$_instRootPath".old
        if [ $? -ne 0 ] ; then
            printf "%s: rm -rf FAILED (%s)\n" $_myfunc "$_instRootPath".old
            return 1
        fi
    fi
    if [  -d "$_rpmDloadPath" ] ; then
        printf "%s: Purge YUM downloads (%s)\n" $_func "$_rpmDloadPath"
        check_path_safe "$_rpmDloadPath" "$_func:$_myfunc"
        if [ $? -ne 0 ] ; then
            return 1
        fi
        find $_rpmDloadPath -name '*.rpm' -exec rm -f {} \; &> /dev/null
        if [ $? -ne 0 ] ; then
            printf "%s: find #1 FAILED (%s)\n" $_myfunc "$_rpmDloadPath"
            return 1
        fi
        find $_rpmDloadPath -name '*.tmp' -exec rm -f {} \; &> /dev/null
        if [ $? -ne 0 ] ; then
            printf "%s: find #2 FAILED (%s)\n" $_myfunc "$_rpmDloadPath"
            return 1
        fi
    fi
    return 0
...
function gen_yum_conf {
   ...
   local _func="gen_yum_conf_from_file"
   local _yumConfBase=''
...
function gen_yum_conf_from_array {
  local _func=gen_yum_conf_from_array
  local _arrayDef=$1
  local _outfile=$2
  local _append=$3
...
function move_groups_to_front {
  local _func='move_groups_to_front'
  local _listName=$1
...
#          and mkiso.sh create, do not care as what epo definitions are used, as
#          as long as they work.
# 
function yum_download {
  local _func="yum_download"
  local _argsName=$1
  local _doInstall=$2
  local _astr=''
  local _args
  ...
  # cleanup 'yum chroot' install state
  purge_yum_state _args $_func
  if [ $? -ne 0 ] ; then
      return 1
  fi
...
function rpm_download {
	...
	if [ ! -f "$_rpmDloadPath/$_rpm.rpm" ] ; then
	    printf "%s: Missing RPM file: %s\sn" $_func "$_file"
	fi
    done
...
function populate_kickstart {
  ...
  local _func="populate_kickstart"
...
function populate_template_file {
  ...
  local _func="populate_template_file"
...
function copy_iso {
    ...
    local _rsync_args="-az --exclude TRANS.TBL"
...
function move_rpms_to_staging {
   local _curdir=`pwd`
   local _kerpath=''
   local _func='move_rpms_to_staging'
...
#
# iso_to_packageless_archive:
#
function iso_to_packageless_archive {
    echo "TODO"
}
...
function test_iso_install {
    ...
    sudo mount -o loop "$_iso_path" $_iso_mount
    if [ $? -ne 0 ] ; then
...
#
function print_help {
   local _func='print_help'
   local _argHelpName=$1
...
function process_create_args {
    local _func='process_create_args'
    local _aStrName=$1
    local _aDefName=$2
    local _aHelpName=$3
    local _defs
...
function process_rpm_to_iso_args {
    local _func='process_rpm_to_iso_args'
    local _aStrName=$1
    local _aDefName=$2
    local _aHelpName=$3
    local _defs
    ...
    if [ ! -z "$_action" -a ! -z "$_mypath" ] ; then
        if [ $_action == 'delete' -o $_action == 'prep' ] ; then
	     _output=`ls -l $WORKSPACE`
             tprint "$DEBUG_FLAG" "%s\n" "$_output"
	     check_path_safe "$_mypath" $_func
	     if [ $? -ne 0 ] ; then
		 printf "%s: %s Failed safety check\n" $_func $_mypath 
                 return 1
             fi
             rm -rf "$_mypath/*"
	     if [ $? -ne 0 ] ; then
		 printf "%s: Failed to remove %s children\n" $_func $_mypath
                 return 1
             fi
	     printf "%s: ... Purged %s\n" $_func $_mypath
        fi
...
function default_mkiso_values {
   local _func='default_mkiso_values'
   local _aStrName=$1
   local _argVals
   local _aStr
   ...
   build_path ISO_STAGING_PATH _aStr iso_staging_path $WORKSPACE iso-staging delete
...
function do_cmd_show_params {
    local _func='do_cmd_show_params'
    tprint "$DEBUG_FLAG" "%s:: %s\n" $_func "$*"
